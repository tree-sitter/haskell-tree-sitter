...
prop_TSNode_sizeOf =
  property $ sizeOf (undefined :: TSNode) === fromIntegral sizeof_tsnode
...
prop_Node_sizeOf =
  property $ sizeOf (undefined :: Node) === fromIntegral sizeof_node

prop_Node_roundtrips = property $ do
  peeked <- liftIO
    (with
      (Node (TSNode 1 (TSPoint 2 3) 4 nullPtr nullPtr)
            nullPtr
            1
            (TSPoint 4 5)
            7
  ...
  peeked === Node (TSNode 1 (TSPoint 2 3) 4 nullPtr nullPtr)
                  nullPtr
                  1
                  (TSPoint 4 5)
                  7
...
prop_TSTreeCursor_sizeOf =
  property $ sizeOfCursor === fromIntegral sizeof_tstreecursor
...
foreign import ccall unsafe "src/bridge.c ts_query_new_p" ts_query_new_p :: CSize
